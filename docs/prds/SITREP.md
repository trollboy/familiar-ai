...
| | |
|---|---|
| PRDs written | ~123 (97 done, 22 queued PRDs plus 4 documents in `docs/prds/`) |
| Driver attempts in this host's store | 43 — **30 on this repository, 13 on `~/Projects/spectra`** (corrected 2026-09-22) |
| Attempts that completed (this repository) | **2** |
| Commits from Familiar's own merge queue, all-time | **5** — the ledger credits 2, because the resume-landing path never writes `integrated_at` |
| Rounds completed unattended | **0** |

Filter every ledger query by `driver_sessions.repository_key`. The
2026-09-17 histogram in the execution plan and PRD-098 counted 12 spectra
fixture rows as this project's two largest stop classes.
...
**PRD-100 is merged** (`e16fc73`, 2026-09-22, local until pushed). Its
review cycle inside Familiar never completed — the package was 19 files /
~960KB against a 250KB host cap — so it was reviewed by hand instead: four
findings fixed on the branch before merge (`7c6d92e`, `703f68c`), gate green
and recorded. A hand landing, recorded as such; PRD-105 carries the
review's non-blocking findings.
...
- **The queue overstates itself.** PRD-082, 085, 090, 095, 096, 099 and
  102 are landed but still in `docs/prds/`. **PRD-097 is not landed**:
  merge `deec7f8` carried only the document, and `delivery.rs` still spells
  six `"pr"` verbs. An earlier cut of this file listed it as landed. The
  ledger disagrees with both the filesystem and GitHub, in different
  directions.
...
Each is individually defensible. Each acquired a veto.

**Correction, 2026-09-22.** An earlier cut said no PRD was ever written
that says the loop completes end to end. Five were: PRD-017, 066, 077,
100 and 101 each state it. PRD-077's sixth criterion, verbatim, ends
"proven in an integration regression with fake agents", and every one of
the five was satisfied the same way. The composition has been tested only
against fakes; it has never been an acceptance criterion against
`driver_attempts`. PRD-107's fifth criterion is the first that is.

Of the ~80 refusal states, 24 have ever fired on this host, two are
unreachable code, and on this repository about six mechanisms did all the
stopping — scope authorization, infrastructure-caused verification
failure, two adapter bugs, a checkpoint bug, reviewer incapacity — almost
always on work later landed by hand unchanged. The audit of the queue
against the owner's 1.0 definition is in `EXECUTION-PLAN.md` under
2026-09-22.
