...
The complete [CLI reference](docs/guides/cli.md) and [configuration reference](docs/guides/configuration.md) are maintained as independent contracts. Product changes update the owning guide instead of this project overview.
...
single definition of what verification means here. There is no second list of
steps to keep in sync, and `gate_contract.rs` fails the build if one appears.

Verification is local — there is no hosted runner. Install the hook once and
it runs before anything leaves the machine, recording its verdict:
...
```bash
familiar-ai ops gate status              # HEAD of the default branch
familiar-ai ops gate status --commit SHA
```
