...
use familiar_mcp::storage::{SqliteStorage, Storage};
use familiar_mcp::tool::{ToolContext, ToolRegistry};
use familiar_mcp::tools::register_default_tools;
use familiar_mcp::transport::MockTransport;
use familiar_mcp::McpServer;
...
fn build_server(
    ...
    McpServer,
    std::sync::Arc<std::sync::Mutex<Vec<familiar_mcp::TransportEvent>>>,
    i64,
) {
...
async fn full_session_handshake_list_call() {
    ...
    let writes: Vec<&str> = events
        ...
        .filter_map(|e| match e {
            familiar_mcp::TransportEvent::Write(s) => Some(s.as_str()),
            _ => None,
        })
        .collect();
    ...
    assert_eq!(init_resp["result"]["serverInfo"]["name"], "familiar-mcp");
...
async fn remember_result_then_get_recent_decisions_via_repo_root() {
    ...
    let writes: Vec<String> = events
        .iter()
        .filter_map(|e| match e {
            familiar_mcp::TransportEvent::Write(s) => Some(s.clone()),
            _ => None,
        })
        .collect();
