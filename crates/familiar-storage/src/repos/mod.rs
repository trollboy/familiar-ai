...
pub(crate) fn parse_dt(s: &str) -> familiar_core::Result<DateTime<Utc>> {
    DateTime::parse_from_rfc3339(s)
        .map(|dt| dt.with_timezone(&Utc))
        .map_err(|e| FamiliarError::Database(format!("failed to parse timestamp '{s}': {e}")))
}

pub(crate) fn json_to_vec(s: &str) -> familiar_core::Result<Vec<String>> {
    serde_json::from_str(s)
        .map_err(|e| FamiliarError::Database(format!("failed to parse JSON array: {e}")))
}

pub(crate) fn vec_to_json(v: &[String]) -> familiar_core::Result<String> {
