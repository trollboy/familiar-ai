...
use familiar_core::models::{Decision, FileSummary, SessionRollup};
...
fn count_query(db: &Database, sql: &str) -> familiar_core::Result<usize> {
    let count: i64 = db
        .conn()
...
pub fn global_stats(db: &Database) -> familiar_core::Result<GlobalStats> {
    Ok(GlobalStats {
...
pub fn projects_with_counts(db: &Database) -> familiar_core::Result<Vec<ProjectWithCounts>> {
    let mut stmt = db
        .conn()
...
pub fn recent_decisions(db: &Database, limit: usize) -> familiar_core::Result<Vec<Decision>> {
...
pub fn recent_session_rollups(
    db: &Database,
    limit: usize,
) -> familiar_core::Result<Vec<SessionRollup>> {
...
mod tests {
    ...
    use familiar_core::models::{NewDecision, NewFileSummary, NewProject};
