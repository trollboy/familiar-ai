...
use familiar_ai_core::config::SummaryConfig;
use familiar_ai_core::models::NewFileSummary;
use familiar_ai_core::CanonicalFileIdentity;
use familiar_ai_storage::{
    Database, FileSummaryRepository, LifecycleRepository, ProjectRepository,
};
use familiar_ai_summary::SummaryGenerator;
    ...
    for result in walker {
        ...
        if classification == "excluded" {
            let _ = db.lock().unwrap().retire_absent(
                project_id,
                identity.path(),
                familiar_ai_storage::RetirementReason::Ineligible,
                "scan",
                Some(run.id),
            );
        }
