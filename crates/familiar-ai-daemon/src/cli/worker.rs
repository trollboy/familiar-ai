...
pub fn run(command: WorkerCommand) -> Result<(), String> {
    match command {
        WorkerCommand::Install { repository } => {
            let (spec, repository, paths) = spec(&repository)?;
            let changed =
        ...
        WorkerCommand::Status { repository } => {
            let (spec, repository, _) = spec(&repository)?;
        ...
        WorkerCommand::Run {
            ...
            let summary = crate::drive_command(Some(max_prds), None, None, None, None, Vec::new())?;
            crate::report_command(Some(&summary.session_id))?;
...
fn spec(
    ...
    let config = crate::effective_repository_config(&paths, &repository)?;
    let spec =
