...
//! bootstrap, and recover or complete individual backlog entries.

use std::path::PathBuf;

use familiar_ai_core::{
    load_manifest, resolve_run_prd, validate_graph, validate_recovery_attribution,
...
pub fn backlog(command: BacklogCommand) -> Result<(), String> {
    let paths = familiar_ai_core::AppPaths::resolve().map_err(|e| e.to_string())?;
    let cwd =
