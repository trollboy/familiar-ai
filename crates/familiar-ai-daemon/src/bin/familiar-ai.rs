...
use familiar_ai_daemon::config_cli::ConfigCommand;
...
fn main() -> ExitCode {
    ...
    match cli.command {
        ...
        Command::Config { command } => {
            match familiar_ai_daemon::config_cli::config_command(command) {
                Ok(()) => ExitCode::SUCCESS,
                Err(error) => fail(error),
            }
        }
        ...
        Command::Preflight => match familiar_ai_daemon::cli::driver::preflight_command() {
            Ok(()) => ExitCode::SUCCESS,
            Err(error) => fail(error),
        },
        Command::Next => match familiar_ai_daemon::cli::driver::next() {
            Ok(()) => ExitCode::SUCCESS,
            Err(error) => fail(error),
        },
        Command::Plan {
        ...
        Command::Run { prd_path } => match familiar_ai_daemon::cli::driver::run(&prd_path) {
            Ok(()) => ExitCode::SUCCESS,
            Err(error) => {
        ...
        Command::Resume { prd, dry_run } => {
            match familiar_ai_daemon::cli::driver::resume_command(&prd, dry_run) {
                Ok(()) => ExitCode::SUCCESS,
                Err(error) => fail(error),
            }
        }
        Command::ScopeDecisions {
            ...
            actor,
            reason,
        } => match familiar_ai_daemon::cli::driver::scope_decisions(
            finding_hash,
            candidate_hash,
            approve,
            reject,
        ...
        Command::Drive {
            ...
            worktree_root,
            prd,
        } => match familiar_ai_daemon::cli::driver::drive_command(
            max_prds,
            max_cost_microusd,
            max_duration_ms,
            max_parallel_components,
        ...
        Command::History { limit, verbose } => {
            match familiar_ai_daemon::cli::billing::history(limit, verbose) {
                Ok(()) => ExitCode::SUCCESS,
                Err(error) => fail(error),
            }
        }
        Command::Usage {
            ...
            filters,
            dense,
        } => match familiar_ai_daemon::cli::billing::usage(
            start.as_deref(),
            end.as_deref(),
            &bucket,
            group_by,
        ...
        Command::Report { session_id } => {
            match familiar_ai_daemon::cli::delivery::report_command(session_id.as_deref()) {
                Ok(()) => ExitCode::SUCCESS,
                Err(error) => fail(error),
            }
        }
        Command::Deliver {
            ownership_record,
            to,
        } => match familiar_ai_daemon::cli::delivery::deliver_command(
            &ownership_record,
            to.as_deref(),
        ) {
            Ok(()) => ExitCode::SUCCESS,
