...
use familiar_ai_daemon::cli::backlog::{backlog, next, BacklogCommand};
use familiar_ai_daemon::cli::billing::{billing_command, BillingCommand};
use familiar_ai_daemon::cli::compress::{compress_command, CompressCommand};
use familiar_ai_daemon::cli::config::{config_command, status_command, ConfigCommand};
use familiar_ai_daemon::cli::control::{control_command, ControlCommand};
use familiar_ai_daemon::cli::deliver::deliver_command;
use familiar_ai_daemon::cli::drive::drive_command;
use familiar_ai_daemon::cli::onboard::{onboard, OnboardCommand};
use familiar_ai_daemon::cli::plan::{plan, PlanCommand};
use familiar_ai_daemon::cli::preflight::preflight_command;
use familiar_ai_daemon::cli::report::report_command;
use familiar_ai_daemon::cli::run::{resume_command, run};
use familiar_ai_daemon::cli::scope::scope_decisions;
use familiar_ai_daemon::cli::stewardship::{stewardship_command, StewardshipCommand};
use familiar_ai_daemon::cli::usage::{history, usage};
use familiar_ai_daemon::cli::worker::{worker_command, WorkerCommand};
...
fn main() -> ExitCode {
    ...
    match cli.command {
        ...
        Command::Compress { command } => match compress_command(command) {
            Ok(()) => ExitCode::SUCCESS,
            Err(error) => fail(error),
        },
        ...
        Command::Status { repository } => match status_command(repository) {
            Ok(()) => ExitCode::SUCCESS,
            Err(error) => fail(error),
        },
