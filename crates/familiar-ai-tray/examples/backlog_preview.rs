...
//!
//! This is a GTK visual fixture, so it intentionally has no macOS UI body.
...
mod linux {
    ...
    impl DataSource for Canned {
        fn query(&self, query: Query) -> Result<Value, String> {
            Ok(match query {
                Query::Repositories => {
                    json!({"repositories": [{"path": "/home/you/Projects/demo"}]})
                }
                Query::Backlog { .. } => {
...
fn main() {
    eprintln!("backlog_preview is a Linux GTK visual fixture");
}
