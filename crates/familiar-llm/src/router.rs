...
impl InferenceRouter {
    ...
    pub fn choose_packer_profile(&self, task: &str) -> familiar_core::config::BudgetProfile {
...
mod tests {
    use super::*;
    use familiar_core::config::{EmbeddingInferenceConfig, InferenceConfig, TextInferenceConfig};
    ...
    async fn choose_packer_profile_heuristic() {
        ...
        assert_eq!(
            ...
            familiar_core::config::BudgetProfile::Minimal
        );
        let long = "word ".repeat(600);
        assert_eq!(
            router.choose_packer_profile(&long),
            familiar_core::config::BudgetProfile::MaxAccuracy
        );
    }
