...
mod driver;
mod execution;
mod familiar_toml; // identity-gate: allow
mod inference;
mod preflight;
mod providers;
mod registry;
...
pub use driver::*;
pub use execution::*;
pub use familiar_toml::*; // identity-gate: allow
pub use inference::*;
pub use preflight::*;
pub use providers::*;
pub use registry::*;
...
"#,
    ...
    /// PRD-076's equivalence oracle: `config.rs` became a `config/` module
    /// tree behind re-exports with no behavior change. A single monolith
    /// file has no "before" to diff against inside this same binary, so the
    /// proof this test carries is that a fixture spanning most of the split
    /// modules (providers, registry, driver, review, worker, daemon,
    /// inference, watcher, tray, summary/rollup/dashboard/packer, artifacts)
    /// still resolves through the real `Config::load` figment/TOML pipeline
    /// to one pinned effective configuration — any wiring mistake introduced
    /// by the split (a dropped `pub use`, a field routed to the wrong
    /// module) would surface here as a parse failure or a changed field.
    #[test]
    fn pinned_fixture_configuration_resolves_identically_across_the_module_split() {
        let file = tempfile::NamedTempFile::new().unwrap();
        std::fs::write(
            file.path(),
            r#"
...
global_concurrency_ceiling = 2
default_project_concurrency_ceiling = 2
...
[database]
path = "/tmp/prd-076-fixture/familiar.db"
...
[artifacts.qwen-local]
id = "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
state = "degraded-unverified-alias"
runtime_alias = "qwen2.5:3b"
...
[driver]
max_prds_per_session = 5
max_session_cost_microusd = 1000000
max_concurrency = 2
max_parallel_components = 2

[worker]
label = "ai.familiar.worker.fixture"
restart_throttle_secs = 20
max_prds_per_run = 3
...
allowed_paths = ["crates/", "tests/"]

[[review.verification]]
check_id = "tests"
argv = ["/usr/local/bin/cargo", "test"]
working_directory = "."
timeout_ms = 300000
required = true
...
[dashboard]
enabled = true
bind_address = "127.0.0.1:9401"

[packer]
default_profile = "aggressive"
hard_ceiling_tokens = 20000
...
"#,
        ...
        let config = Config::load(Some(file.path())).expect("fixture configuration loads");

        assert_eq!(config.daemon.heartbeat_interval_secs, 45);
        assert_eq!(config.daemon.global_concurrency_ceiling, 2);
        assert_eq!(config.logging.level, "debug");
        assert_eq!(config.logging.format, LogFormat::Json);
        assert_eq!(
            config.database.path.as_deref(),
            Some(Path::new("/tmp/prd-076-fixture/familiar.db"))
        );
        assert!(config.providers.contains_key("unsloth-local"));
        assert_eq!(
            config.providers["unsloth-local"].runtime,
            Some(InferenceRuntimeKind::Unsloth)
        );
        assert!(config.artifacts.contains_key("qwen-local"));
        assert_eq!(config.execution_context.hard_ceiling_tokens, Some(12000));
        assert!(config.execution_context.prompt_cache_enabled);
        let planner = config.planner.as_ref().expect("planner present");
        assert_eq!(planner.max_prds_per_batch, 8);
        assert_eq!(config.driver.max_prds_per_session, 5);
        assert_eq!(config.driver.max_parallel_components, 2);
        assert_eq!(config.worker.restart_throttle_secs, 20);
        assert_eq!(config.worker.max_prds_per_run, 3);
        let registry = config.worker_registry.as_ref().expect("registry present");
        assert_eq!(
            registry.routing.implementation_pin.as_deref(),
            Some("codex-primary")
        );
        assert!(registry.workers.contains_key("codex-primary"));
        assert!(config.review.enabled);
        assert_eq!(config.review.max_review_attempts, 4);
        assert_eq!(
        ...
        assert_eq!(config.summary.staleness_threshold_secs, 43200);
        assert_eq!(config.rollup.default_limit, 10);
        assert!(config.dashboard.enabled);
        assert_eq!(config.packer.default_profile, BudgetProfile::Aggressive);
        assert_eq!(config.inference.text.mode, InferenceMode::LocalOnly);
        assert_eq!(config.inference.embedding.provider, ProviderKind::Builtin);

        // Re-resolving from the identical bytes is deterministic — the
        // module split introduced no hidden mutable or ordering-dependent
        // state anywhere along the figment merge chain. `Config` has no
        // `PartialEq` (it never needed one before this split either), so
        // re-check a representative field from each split module rather
        // than a whole-struct comparison.
        let reloaded = Config::load(Some(file.path())).expect("fixture reloads");
        assert_eq!(reloaded.daemon.heartbeat_interval_secs, 45);
        assert_eq!(reloaded.driver.max_parallel_components, 2);
        assert_eq!(reloaded.review.max_review_attempts, 4);
        assert_eq!(reloaded.packer.default_profile, BudgetProfile::Aggressive);
    }
}
