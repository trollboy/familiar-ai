...
use artifacts::hex_digest;
use providers::validate_identifier;
...
impl Config {
    ...
    pub(crate) fn validate_repositories(&self) -> crate::Result<()> {
...
mod tests {
    ...
    #[test]
    fn pinned_default_toml_resolves_to_identical_effective_configuration() {
        // Behavior-preservation oracle for PRD-076's config.rs -> config/ module
        // split: config/default.toml must still resolve to byte-identical JSON
        // after the refactor as it did before.
        //
        // Guards against concurrent env mutation from other config-loading
        // tests in this file, which run in parallel threads within the same
        // test binary and would otherwise transiently leak env overrides into
        // this test's unqualified `Config::load`.
        let _guard = BUDGET_ENV.lock().unwrap();
        let manifest_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
        let repo_root = manifest_dir.join("../..");
        let path = repo_root.join("config/default.toml");
        let config = Config::load(Some(&path)).expect("pinned fixture loads");
        let actual = serde_json::to_string_pretty(&config).expect("serializes");
        let expected = r#"{
  "repositories": {},
  "providers": {},
  "artifacts": {},
  ...
  "daemon": {
    ...
    "global_concurrency_ceiling": 1,
    "default_project_concurrency_ceiling": 1,
...
"#,
    ...
    fn env_overrides_file() {
        // Guard against concurrent env mutation from other config-loading
        // tests in this file (e.g. the pinned default-fixture test), which
        // run in parallel threads within the same test binary.
        let _guard = BUDGET_ENV.lock().unwrap();
