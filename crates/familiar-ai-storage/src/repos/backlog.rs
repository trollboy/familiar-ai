...
fn reconcile_prd(
    ...
    // A moved PRD leaves its old path behind as a missing row, and that is
    // left alone deliberately.
    //
    // Carrying the row to the new path or deleting it both look obvious and
    // both are wrong: `backlog_status_events` and `backlog_bootstrap_items`
    // hold foreign keys into `(repository_key, prd_path)`, so updating the
    // path orphans the history and deleting the row is refused outright. An
    // attempt at each aborted the whole reconciliation transaction and broke
    // `next`, `run` and `drive`.
    //
    // Consolidating these rows needs the references to move with them — an
    // ON UPDATE CASCADE, which is a migration and a piece of work in its own
    // right, not something to slip into reconciliation.
...
mod tests {
    ...
    /// Reconciliation must survive a row that other tables reference.
    ///
    /// `backlog_status_events` and `backlog_bootstrap_items` both have foreign
    /// keys into `backlog_prds`, so any attempt to tidy an old row away aborts
    /// the whole transaction and takes `next`, `run` and `drive` with it. A
    /// first attempt at clearing moved-from rows did exactly that.
    #[test]
    fn reconciliation_survives_rows_other_tables_reference() {
        let mut db = Database::open_in_memory().unwrap();
        db.run_migrations().unwrap();
        {
            let mut storage = SqliteBacklogRepository::new(db.conn_mut());
            storage.reconcile_and_snapshot(&repo(), &[prd()]).unwrap();
            // Give the row the history a real one accumulates.
            storage
                .connection
                .execute(
                    "INSERT INTO backlog_status_events(repository_key,prd_path,old_status,\
                     new_status,actor,changed_at) VALUES(?1,?2,'pending','in_progress','human:x','t')",
                    params![repo().key, prd().path.as_str()],
                )
                .unwrap();
        }

        // Archive it: the move must reconcile without tripping the key.
        let mut archived = prd();
        archived.path = RepositoryPath::new("docs/prds/done/PRD-009.md").unwrap();
        archived.location = PrdLocation::Archived;
        {
            ...
            storage
                .reconcile_and_snapshot(&repo(), &[archived])
                .expect("reconciliation must not fail on a referenced row");
        }

        // And the run after it, which is where a broken transaction shows up.
        let mut storage = SqliteBacklogRepository::new(db.conn_mut());
        storage
            .reconcile_and_snapshot(&repo(), &[prd()])
            .expect("a second reconciliation must still succeed");
    }
