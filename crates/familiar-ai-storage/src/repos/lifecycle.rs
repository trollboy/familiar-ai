use familiar_ai_core::{models::NewFileSummary, CanonicalFileIdentity, FamiliarError};
...
pub trait LifecycleRepository {
    fn observe_change(
        ...
        source: &str,
        deferred: bool,
    ) -> familiar_ai_core::Result<LifecycleOutcome>;
    fn retire_absent(
        &self,
        project_id: i64,
        path: &str,
    ...
    fn list_pending_summary_work(
        ...
        project_id: i64,
        limit: usize,
    ) -> familiar_ai_core::Result<Vec<PendingSummaryWork>>;
    fn complete_summary_work(&self, project_id: i64, path: &str) -> familiar_ai_core::Result<()>;
    fn commit_summary_work(&self, summary: &NewFileSummary) -> familiar_ai_core::Result<()>;
    ...
    fn latest_scan_status(&self, project_id: i64) -> familiar_ai_core::Result<Option<ScanStatus>>;
}

fn db_error(e: rusqlite::Error) -> FamiliarError {
    FamiliarError::Database(e.to_string())
}
fn validate(project_id: i64, path: &str) -> familiar_ai_core::Result<()> {
...
fn next_order(tx: &Transaction<'_>, project_id: i64) -> familiar_ai_core::Result<i64> {
...
fn ensure_project_ready(tx: &Transaction<'_>, project_id: i64) -> familiar_ai_core::Result<()> {
...
fn preserve_and_delete(
    ...
    related: Option<&str>,
    order: i64,
) -> familiar_ai_core::Result<Option<i64>> {
    let now = now_rfc3339();
...
impl Database {
    pub fn start_repository_scan(
        ...
        root: &str,
        policy_version: &str,
    ) -> familiar_ai_core::Result<ScanRun> {
        let tx = self.conn().unchecked_transaction().map_err(db_error)?;
        ensure_project_ready(&tx, project_id)?;
    ...
    pub fn fail_repository_scan(&self, run_id: i64, error: &str) -> familiar_ai_core::Result<()> {
    ...
    pub fn mark_scan_enumeration_complete(&self, run_id: i64) -> familiar_ai_core::Result<()> {
    ...
    pub fn reconcile_repository_scan(&self, run: &ScanRun) -> familiar_ai_core::Result<()> {
