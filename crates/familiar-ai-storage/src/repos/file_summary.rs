use familiar_ai_core::models::{FileSummary, NewFileSummary};
use familiar_ai_core::CanonicalFileIdentity;
...
pub trait FileSummaryRepository {
    ...
    ) -> familiar_ai_core::Result<FileSummary>;
    fn get_file_summary_by_path(
        &self,
        project_id: i64,
        path: &str,
    ) -> familiar_ai_core::Result<Option<FileSummary>>;
    fn list_file_summaries_by_project(
        &self,
        project_id: i64,
    ) -> familiar_ai_core::Result<Vec<FileSummary>>;
    fn delete_file_summary(&self, id: i64) -> familiar_ai_core::Result<()>;
}
...
impl Database {
    ...
    pub fn reconcile_file_summary_identities(
        &self,
        project_id: i64,
    ) -> familiar_ai_core::Result<FileSummaryReconciliationResult> {
    ...
    fn start_file_summary_reconciliation(&self, project_id: i64) -> familiar_ai_core::Result<i64> {
    ...
    fn record_failed_file_summary_reconciliation(
        &self,
        run_id: i64,
    ...
    pub fn latest_file_summary_reconciliation(
        &self,
        project_id: i64,
    ) -> familiar_ai_core::Result<Option<FileSummaryReconciliationResult>> {
    ...
    pub fn rollback_file_summary_reconciliation(
        &self,
        run_id: i64,
    ) -> familiar_ai_core::Result<FileSummaryRollbackResult> {
        let tx = self
            .conn()
...
fn load_project_rows(
    ...
    project_id: i64,
) -> familiar_ai_core::Result<Vec<StoredFileSummary>> {
...
fn load_prior_reconciliations(
    connection: &rusqlite::Connection,
    project_id: i64,
) -> familiar_ai_core::Result<Vec<PriorReconciliation>> {
    let mut statement = connection
        .prepare(
...
fn load_run_record_summary(
    connection: &rusqlite::Connection,
    run_id: i64,
) -> familiar_ai_core::Result<(BTreeMap<&'static str, usize>, Vec<i64>)> {
...
fn stable_reason(reason: &str) -> familiar_ai_core::Result<&'static str> {
    match reason {
...
fn rollback_run(
    tx: &Transaction<'_>,
    run_id: i64,
) -> familiar_ai_core::Result<Result<FileSummaryRollbackResult, String>> {
...
fn load_summary_by_id(
    connection: &rusqlite::Connection,
    id: i64,
) -> familiar_ai_core::Result<Option<StoredFileSummary>> {
    connection
        .query_row(
            "SELECT id, project_id, path, summary, tags_json, extracted_symbols_json, \
             last_known_mtime, last_known_size, last_updated_at, created_at, updated_at \
...
fn validate_module_prefix(path_prefix: &str) -> familiar_ai_core::Result<()> {
