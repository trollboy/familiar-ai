...
fn bootstrap() -> familiar_core::Result<(DaemonState, familiar_logging::LogGuard)> {
    let cli = Cli::parse();
    ...
    let log_guard = familiar_logging::init_logging(&config.logging, Some(&paths.log_dir))?;
    ...
    let status = Arc::new(Mutex::new(AppStatus::new()));
    {
        ...
        // (if config.inference.text.mode != familiar_core::config::InferenceMode::Disabled is set).
        ...
        // (familiar-mcp binary) spawned per-session by the client.
        s.mcp_enabled = true;
        tracing::info!(
...
async fn daemon_run(
    ...
    if state.config.inference.text.mode != familiar_core::config::InferenceMode::Disabled {
...
#[cfg(not(feature = "tray"))]
async fn run_headless() -> familiar_core::Result<()> {
    ...
    tracing::info!("familiar-daemon stopped");
    Ok(())
}
...
fn main() -> ExitCode {
    ...
    let (tray_tx, mut tray_rx) = mpsc::channel::<familiar_tray::TrayCommand>(64);
    ...
    let tray_app = familiar_tray::TrayApp::new(
