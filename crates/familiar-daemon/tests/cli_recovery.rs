...
use familiar_storage::{Database, SqliteBacklogRepository};
...
fn claimed_repo() -> (tempfile::TempDir, std::path::PathBuf) {
    ...
    store
        .transition(
            ...
            "system:familiar-run:00001785772020811891-0000057947-000001",
        )
        .unwrap();
    drop(db);
    (repo, database)
...
fn complete_is_a_human_only_manual_override() {
    let (repo, database) = claimed_repo();
    let rejected = Command::new(env!("CARGO_BIN_EXE_familiar"))
        .args([
            "backlog",
            "complete",
            "docs/prds/PRD-012.md",
    ...
    let accepted = Command::new(env!("CARGO_BIN_EXE_familiar"))
        .args([
            "backlog",
            "complete",
            "docs/prds/PRD-012.md",
